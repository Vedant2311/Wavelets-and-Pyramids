imageA = imread('ba1.png');
imageB = imread('ba2.png');
mask = imread('ba3.png');

imageA_R = imageA(:,1:257);
imageA_R = double(imageA_R);
imageB_R = imageB(:,1:257);
...
for i=1:9
    for j=1:length(image_laplacianA_R{i})
        for k=1:length(image_laplacianA_R{i})
           % mask_gaussian{i}(j,k)
            LS_R{i}(j,k) = ((mask_gaussian{i}(j,k)/255)*image_laplacianA_R{i}(j,k)) + ((1-(mask_gaussian{i}(j,k))/255)*image_laplacianB_R{i}(j,k));
        end
    end
end
